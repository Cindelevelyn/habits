...
export function HabitsList({ date, onCompletedChanged }: HabitsListProps) {
  ...
  useEffect(() => {
    ...
    }).then(res => setHabitsInfo(res.data)) //recebe os dados do BD
  }, [])
      ...
      {habitsInfo?.possibleHabits.map((habit) => {
        return (
          <Checkbox.Root
            ...
            <span className='font-semibold text-xl leading-tight text-white group-data-[state=checked]:line-through group-data-[state=checked]:text-zinc-400'>{habit.title}</span>
          </Checkbox.Root>
        )
      })}
